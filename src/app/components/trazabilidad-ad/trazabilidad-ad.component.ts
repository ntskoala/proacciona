...
initTreeMateriaPrima(){
    ...
    //      )
         ...
         this.getChildren(this.tree[0],this.materiaPrima.id,'idmateriaprima',1,this.tree[0]);
...
initTreeAdelante(){
    ...
    this.nodozero = false;
     ...
     //this.tree[0].children=[];
     this.tree.push({"label": this.orden.numlote,
      "data":{"tipo":"orden","idOrden":this.orden.id,"fecha_inicio_orden":this.orden.fecha_inicio,"level":0,"almacen":this.orden.idalmacen,"cantidad":this.orden.cantidad,"cantidad_remanente":this.orden.remanente,"cliente":this.orden.idcliente,"fecha_caducidad":this.orden.fecha_caducidad,"tipo_medida":this.orden.tipo_medida,"nombreProduccion":this.orden.nombre},
     "expanded":true,"expandedIcon": "fa-folder-open","collapsedIcon": "fa-folder"})
      //this.setItems(this.tree[0],this.orden.id,0);

      this.getChildren(this.tree[0],this.orden.id,'idloteinterno',1,this.tree[0]);
          ...
          response => {
            ...
            if (response.success && response.data) {
              ...
              for (let element of response.data) {
                      ...
                      "label":element.numlote,
                      "expanded":true,
                      "data":{"tipo":"orden","idOrden":element.idorden,"fecha_inicio_orden":element.fecha_inicio,"idDetalleOrden":element.id,"numlote_proveedor":element.numlote_proveedor,"level":level,"almacen":element.idalmacen,"cantidad":element.cantidad_detalle,"remanente":element.remanente,"cliente":element.idcliente,"fecha_caducidad":element.fecha_caducidad,"cantidad_remanente_origen":element.cantidad_remanente_origen,"cantidad_real_origen":element.cantidad_real_origen,"tipo_medida":element.tipo_medida,"nombreProduccion":element.nombreProduccion,"fecha_detalle":element.fecha_detalle,"parent": parent}
                        });

                        this.getChildren(nodo.children[i],element.idorden,'idloteinterno',level,nodo.children[i]);
                        i++;
                      //}
