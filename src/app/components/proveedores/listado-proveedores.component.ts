...
public  nuevoItem: Proveedor = new Proveedor('',0,0);
public item1:Proveedor = new Proveedor('Selecciona',0,0);
          ...
          response => {
            ...
            if (response.success == 'true' && response.data) {
              for (let element of response.data) {
                this.items.push(new Proveedor(element.nombre,element.idempresa,element.id));
              }
    ...
    this.servidor.postObject(URLS.STD_ITEM, proveedor,param).subscribe(
      response => {
        if (response.success) {
          ...
          this.nuevoItem = new Proveedor('',0,0);
        }
    });
}
