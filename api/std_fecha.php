...
switch ($method) {
  ...
  case 'PUT':
   // $sql = "update `$table` set fecha_ = IF(fecha_ < '$values[0]','$values[0]', fecha_ ) where id=$key"; break;
    $sql = "update `$table` set $columns[0] = IF($columns[0] < '$values[0]','$values[0]', $columns[0] ) where id=$key"; break;
...
{ 
  $nuevaFecha = $reg[$columns[0]];
}
$result = '{"success":"nuevaFecha","nuevaFecha":"' . $nuevaFecha .'"}';
}else{
